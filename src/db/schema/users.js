// schema.ts
import { mysqlTable, varchar, timestamp, int, boolean, text, decimal, date, time, json } from 'drizzle-orm/mysql-core';
...
export const classes = mysqlTable('classes', {
  ...
  classTeacherId: varchar('class_teacher_id', { length: 36 }),
  status: varchar('status', { length: 20 }).default('active'),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow().onUpdateNow(),
});
...
export const attendance = mysqlTable('attendance', {
  ...
  markingMethod: varchar('marking_method', { length: 20 }).default('manual'), // manual, qr
  createdAt: timestamp('created_at').defaultNow(),
});
...
export const feePenalties = mysqlTable('fee_penalties', {
  ...
  penaltyPerDay: decimal('penalty_per_day', { precision: 12, scale: 2 }).notNull(),
  createdAt: timestamp('created_at').defaultNow(),
});
...
export const mcqTests = mysqlTable('mcq_tests', {
  ...
  negativeMarking: decimal('negative_marking', { precision: 5, scale: 2 }).default('0'),
...
export const results = mysqlTable('results', {
  ...
  obtainedMarks: decimal('obtained_marks', { precision: 10, scale: 2 }).notNull(),
  percentage: decimal('percentage', { precision: 5, scale: 2 }).notNull(),
...
// Permission Table (FIXED: Removed duplicate 'classes' column)
...
// Payment Gateway Table (FIXED: Corrected column names)
